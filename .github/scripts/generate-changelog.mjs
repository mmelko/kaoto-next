...
function getTagDate(tag) {
  // For annotated tags use the tagger date; for lightweight tags use commit date
  const date =
    runGit(['log', '-1', '--format=%aI', tag]) ??
    runGit(['log', '-1', '--format=%cI', `${tag}^{commit}`]);
  return date ?? null;
}
...
async function fetchMergedPRs(repo, token, since) {
  ...
  while (true) {
    ...
    if (!res.ok) {
      console.error(`Warning: GitHub API error ${res.status} fetching PRs — stopping pagination.`);
      break;
    }
    ...
    for (const pr of batch) {
      ...
      // Stop if we've gone past the previous tag date
...
const repo = env.GITHUB_REPOSITORY ?? remoteUrl.match(/[:/]([^/:]+\/[^/.]+?)(\.git)?$/)?.[1] ?? null;
...
if (!token || !repo) {
  ...
  const logOutput = runGit(['log', range, '--pretty=format:%H%x09%s%x00', '--']);
